...
class EGSCTeacher(torch.nn.Module):
    def __init__(self, in_channels,args):
        super(EGSCTeacher, self).__init__()
        D = 16
        self.args = {
                "out_channels" : 1,
                "filters_1": 64,
        ...
        #3 global attention
        self.global_attention = nn.ModuleList([global_attention_readout(filters) for filters in [self.args['filters_1'],self.args['filters_2'],self.args['filters_3']]])
        #4 EFN
        self.EFNs = nn.ModuleList(
            [EFN(2*filters,self.args['d'])  for filters in [self.args['filters_1'],self.args['filters_2'],self.args['filters_3']] ]
        )

        self.EFNs.append(EFN(self.args['d']*3,D))
        self.fc = nn.Sequential(
            nn.Linear(D,D//2),
            nn.ReLU(inplace=True),
            nn.Linear(D//2,1),
    ...
    def joint_embeddings(self,x_i,x_j,edge_index_i,edge_index_j,x_i_batch,x_j_batch):
        ...
        joint_embeddings1 = self.EFNs[0](self.global_attention[0](xi1,x_i_batch),self.global_attention[0](xj1,x_j_batch))

        xi2 = self.conv_2(xi1, edge_index_i)
        xj2 = self.conv_2(xj1, edge_index_j)
        joint_embeddings2 = self.EFNs[1](self.global_attention[1](xi2,x_i_batch),self.global_attention[1](xj2,x_j_batch))

        xi3 = self.conv_3(xi2, edge_index_i)
        xj3 = self.conv_3(xj2, edge_index_j)
        joint_embeddings3 = self.EFNs[2](self.global_attention[2](xi3,x_i_batch),self.global_attention[2](xj3,x_j_batch))

        h_ij_all = self.EFNs[3](joint_embeddings1,joint_embeddings2,joint_embeddings3)
        return h_ij_all
    ...
    def forward(self,data1,data2):
        ...
        xi1 = self.conv_1(xi, edge_index_i)
        xj1 = self.conv_1(xj, edge_index_j)
        joint_embeddings1 = self.EFNs[0](self.global_attention[0](xi1,batch_i),self.global_attention[0](xj1,batch_j))

        xi2 = self.conv_2(xi1, edge_index_i)
        xj2 = self.conv_2(xj1, edge_index_j)
        joint_embeddings2 = self.EFNs[1](self.global_attention[1](xi2,batch_i),self.global_attention[1](xj2,batch_j))

        xi3 = self.conv_3(xi2, edge_index_i)
        xj3 = self.conv_3(xj2, edge_index_j)
        joint_embeddings3 = self.EFNs[2](self.global_attention[2](xi3,batch_i),self.global_attention[2](xj3,batch_j))

        h_ij_all = self.EFNs[3](joint_embeddings1,joint_embeddings2,joint_embeddings3)

        return self.fc( h_ij_all ).squeeze(1)
